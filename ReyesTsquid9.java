...
int many=4;
String names[]= { "Blake", "Terrance", "Michael", "Ethan" };
float spacing;

Boat school[]= new Boat[many];
String names[]= { "Pip", "Chip", "Tin", "Gin"};
float spacing;

float surface;
float moonX=50, moonY=200;
int score=50;
...
//// Squids.
void reset() {
  surface= random ( height/10, height/5 );
  moonY= surface/4;
...
void draw() {
  scene();
  if (key >= 'R' && key <= 'T') {
    boatReport ( 75, pip, chip, tin, gin, 4);
    squidReport (surface+75, school, school.length);
  }
  else action();
  show();
  messages();
...
void messages() {
  fill (0);
  textSize( 45 );
  text( "Squid 9", width/4, 22 );
  textSize(14);
  text ( "Hold B key to show all boats and squid", width/4, 42);
  text( "REYES: squids9.java", 15, height-15 );
  if (score>0) text( "SCORE: "+score, width*3/4, 22);
  if (score>750) {
    if ( key == 'q') score=0;
    text ( "Highest score.\nQUITTING NOW\nPress the 'q' ket to continue", width/3, 58 );
    if (score>10000) { exit(); }
  }
}
////MOVE & DRAW.  
void scene() {
  background ( 75, 200, 250 );
  // Moon
  if (moonX > width+125) {
    moonX= -125;
    moonY= random( 125, surface+75 );
  }
  moonX += 1;
  fill ( 250,200,75 );
  ellipse( moonX, moonY-200*sin( PI * moonX/width ), 30, 30 );
  fill( 0, 100, 50 );
  noStroke();
...
void boatReport ( float top, Boat bb, int many ) {
  fill(255,200,200);
